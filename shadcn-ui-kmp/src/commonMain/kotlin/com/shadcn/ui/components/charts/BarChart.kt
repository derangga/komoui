 ...
 * When [showTooltip] is true, press-and-drag horizontally to scrub through
 * categories — a tooltip appears with all series values for the active column.
 *
...
fun BarChart(
    ...
    val leftInsetPx = with(density) { (if (axisOptions.showY) insets.left else 0.dp).toPx() }
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        Box(
            ...
            val gestureMod = if (showTooltip && columnCount > 0) {
                Modifier.chartScrub(
                    columnCount = columnCount,
                    plotLeft = leftInsetPx,
                    plotWidth = max(0f, canvasSize.width.toFloat() - leftInsetPx),
                ) { idx, pos ->
                    scrub.index = idx
                    scrub.pointer = pos
                }
            ...
            Canvas(
                ...
                val plotRect = computePlotRect(size, PlotInsets(leftInsetPx, bottomInsetPx))
                ...
                val baselineY = plotRect.bottom - ((0f - domain.start) / span) * plotRect.height
                ...
                for (col in 0 until columnCount) {
                    ...
                    series.forEachIndexed { si, s ->
                        ...
                        val targetY = plotRect.bottom - ((point.y - domain.start) / span) * plotRect.height
            ...
            if (activeIdx != null && activeIdx in 0 until columnCount) {
                val slotWidthPx = max(0f, canvasSize.width - leftInsetPx) / columnCount
                val centerX = leftInsetPx + slotWidthPx * (activeIdx + 0.5f)
                val xDp = with(density) { centerX.toDp() }
                Box(
                    modifier = Modifier.padding(start = xDp + 8.dp, top = 8.dp),
                ) {
                    ChartTooltip(
